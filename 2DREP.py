...
def calc_sec(molecule = None):
    ...
    for i in range(1, len(identifiers)-1): #a "second pass" filter to correct misplaced annotations
        if identifiers[i] == "&alpha;":
            if identifiers[i-1] == "&nbsp;" and identifiers[i+1] == "&nbsp;":
                identifiers[i] = "&nbsp;"
            elif identifiers[i-1] == "&beta;":
                identifiers[i] = "&beta;"
            elif identifiers[i-1] == "&beta;" and identifiers[i+1] == "&nbsp;":
                identifiers[i] = "&nbsp;"
            elif identifiers[i-1] == "&nbsp;" and identifiers[i+1] == "&beta;":
                identifiers[i] = "&nbsp;"
        elif identifiers[i] == "&beta;":
            if identifiers[i-1] == "&nbsp;" and identifiers[i+1] == "&nbsp;":
                identifiers[i] = "&nbsp;"
            elif identifiers[i-1] == "&alpha;":
                identifiers[i] = "&alpha;"
            elif identifiers[i-1] == "&alpha;" and identifiers[i+1] == "&nbsp;":
                identifiers[i] = "&nbsp;"
            elif identifiers[i-1] == "&nbsp;" and identifiers[i+1] == "&alpha;":
                identifiers[i] = "&nbsp;"
            elif identifiers[i-1] == "&nbsp;" and identifiers[i+1] == "&beta;" and identifiers [i+2] == "&alpha;":
                identifiers[i] = "&alpha;"
            ...
            if monomer.type().trim() in residues: #excludes water and ligands
                if monomer.type().trim() in ("ARG", "LYS"):
                    ...
                    for key, value in ionics[ion_index].items():
                        if monomer.id().trim() in value:
                            if len(str(key)) == 1:
                                identifiers.append("<span style=\"color:white;background-color:#D71313\">" + str(key) + "</span>")
                            else:
                                identifiers.append("<span style=\"font-size:50%;color:white;background-color:#D71313\">" + str(key) + "</span>")
                    if counter != len(identifiers):
                        identifiers.append("&nbsp;")
                    ...
                    identifiers.append("&nbsp;")
        ion_index += 1
        list_of_lists.append(identifiers)
...
def write_data(filename = "2DREP_Output.html", molecule = None):
    ...
    with open(filename, "a") as output:
        for polymer in model:
            ...
            tert_annotations = tert_struc[chain_number]
            chain_number += 1
                ...
                if counter == 0:
                    ...
                    if residue_count+75 < len(sequence):
                        for i in range(residue_count-1, residue_count+75):
                            output.write(tert_annotations[i])
                    else:
